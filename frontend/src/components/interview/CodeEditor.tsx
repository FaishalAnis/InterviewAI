...
export const CodeEditor: React.FC<CodeEditorProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 flex-grow items-stretch">
      {/* Editor & Instructions panel */}
      <div className="lg:col-span-3 flex flex-col glass-panel rounded-2xl p-5 border border-white/5 overflow-hidden">
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-white/5">
          <div className="flex flex-col">
            <h3 className="text-lg font-bold text-white leading-none">{codingMetadata.title}</h3>
            <span className="text-xs text-slate-400 mt-1">Write your algorithm in the editor below.</span>
          </div>
          ...
          <div className="flex items-center space-x-3">
            <select
              ...
              className="bg-slate-900 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-slate-200 focus:outline-none focus:ring-1 focus:ring-primary-500"
            >
        ...
        <div className="text-sm text-slate-300 mb-4 bg-slate-950/30 p-3 rounded-lg border border-white/5 overflow-y-auto max-h-48 whitespace-pre-line">
          {codingMetadata.problem_description}
        </div>

        <div className="flex-grow min-h-[300px] border border-white/5 rounded-xl overflow-hidden shadow-inner">
          <MonacoEditor
            height="100%"
        ...
        <div className="flex items-center justify-between mt-4">
          <span className="text-xs text-slate-400">Code is auto-saved locally</span>
          
          <div className="flex items-center space-x-3">
            <button
              onClick={handleRun}
              disabled={isRunning || isSubmitting}
              className="flex items-center space-x-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-xs font-semibold rounded-lg text-slate-200 border border-white/5 transition disabled:opacity-50"
            >
      ...
      <div className="lg:col-span-2 flex flex-col bg-slate-950/30 rounded-2xl border border-white/5 p-5">
        <h4 className="text-sm font-bold text-white uppercase tracking-wider mb-3">Test Cases & Output</h4>
        ...
        <div className="flex-grow overflow-y-auto flex flex-col space-y-4">
          {results ? (
            <div className="space-y-3">
              {sandboxStatus && (
                <div className={`p-3 rounded-lg border text-xs font-semibold flex items-center space-x-2 ${
                  sandboxStatus === "accepted" ? "bg-emerald-950/20 border-emerald-500/20 text-emerald-400" : "bg-rose-950/20 border-rose-500/20 text-rose-400"
                }`}>
              ...
              {results.map((r, index) => (
                <div key={index} className={`p-3 rounded-xl border ${
                  r.passed ? "bg-emerald-950/10 border-emerald-500/10" : "bg-rose-950/10 border-rose-500/10"
                }`}>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs font-mono pt-1 border-t border-white/5 text-slate-400">
                    <div>
                      <span className="text-[10px] text-slate-500 block">Expected</span>
                      <span className="text-slate-200">{r.expected}</span>
                    </div>
                    <div>
                      <span className="text-[10px] text-slate-500 block">Actual</span>
                      <span className="text-slate-200">{r.output || (r.error ? "Error" : "None")}</span>
                    </div>
                  </div>
                  
                  {r.error && (
                    <div className="mt-2 text-[10px] font-mono text-rose-400 bg-rose-950/25 p-2 rounded border border-rose-500/10 overflow-x-auto whitespace-pre">
                      {r.error}
                    </div>
                  )}
                </div>
            ...
            <div className="p-4 bg-rose-950/20 border border-rose-500/10 rounded-xl text-rose-400 font-mono text-xs flex items-start space-x-2">
              <AlertCircle size={16} className="mt-0.5 shrink-0" />
              <div className="overflow-x-auto whitespace-pre-wrap">{consoleError}</div>
            </div>
          ) : (
            <div className="flex-grow flex flex-col items-center justify-center text-slate-500 text-xs p-6 text-center">
              <Play className="mb-2 text-slate-600" size={32} />
